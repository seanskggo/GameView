...
// Converts Dracula's play. Converts Dracula's special moves based on where move
// is in history
static void convertSpecial(char *currMove, History *curr);
    ...
    if (total == 0) {
        ...
        *numReturnedLocs = total;
        ...
        for (int i = total - 1; i >= 0; i--) {
            ...
            currMove[1] = curr->play[2];
            currMove[2] = '\0';
            // convert currPlace to actual location
            if (player == PLAYER_DRACULA) {
                convertSpecial(currMove, curr);
            }
            // convert to PlaceId
            PlaceId currPlace = placeAbbrevToId(currMove);
...
// Converts Dracula's play. Converts Dracula's special moves.
static void convertSpecial(char *currMove, History *curr) {
	// Else, the following applies to Dracula conversion
	if (strcmp(currMove, "TP") == 0) {
		...
		currMove[0] = tmp[1];
		currMove[1] = tmp[2];
		if (placeAbbrevToId(currMove) >= HIDE && 
		    placeAbbrevToId(currMove) <= TELEPORT) 
	    {
		    History *currHistory = curr->next;
		    convertSpecial(currMove, currHistory);
		}
