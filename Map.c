	...
	ConnList reachableLocs, int *numReturnedLocs, int *added)
{
	...
	for(int i = 0; i < dist; i++) {
		// may need if: i != 0 to get new connections?
		// or could have getConnectionsToCheck, which would factor in
		// the value of i...
		...
		// I think this function (below) is uncessary!!
		// turns out it's not!!! but i have no clue why!!!
		...
		// loop through connsToCheck and add them to queue?
		for (; curr != NULL; curr = curr->next) {
			// if we haven't added the element, and the connection type
			// if rail, then we can add it!!
			if (added[curr->p] == -1 /*&& curr->type == RAIL*/) {
				// add the location to our reachableLocs array!
				reachableLocs = connListInsert(reachableLocs, curr->p,
					curr->type);
				// then set added[curr->p] = 1;
				added[curr->p] = 1;
				// then *numReturnedLocs++;
				*numReturnedLocs += 1;
			}
		}
		// at the end of the for loop, do we need to update connsToCheck??
		// aka we could make
	}
	...
	int added[NUM_REAL_PLACES])
{
	if (iteration == 0) {
		...
		// basically, add all of the connections of that connlist
